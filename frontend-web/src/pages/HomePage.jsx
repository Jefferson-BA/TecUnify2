...
export default function HomePage() {
  ...
  // ESTADO SOLO PARA LA RESERVA (Sin menú extra)
  ...
  const handleReservarClick = (espacio) => {
    setSelectedSpace(espacio); // Abre el formulario
  };

  const handleBackFromReservation = () => {
    setSelectedSpace(null); // Vuelve al inicio
  };
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col h-full">
        ...
        <div className="p-5 flex-1 flex flex-col">
          <h3 className="text-lg font-bold text-gray-900 mb-3 leading-snug" title={espacio.nombre}>
            {espacio.nombre}
          </h3>
          <div className="space-y-3 text-sm text-gray-500 mb-6 flex-1">
            ...
            <div className="flex items-center gap-3 text-green-600 font-medium">
              <span className="text-xs bg-green-50 px-2 py-1 rounded">Precio: S/. {espacio.precioHora || '0.00'}/hora</span>
            </div>
          </div>
          {/* ESTE BOTÓN AHORA SÍ ABRE EL FORMULARIO */}
          <button
            className="w-full bg-[#0099CC] hover:bg-[#0088b5] text-white font-semibold py-2.5 rounded-xl transition-colors shadow-sm"
            onClick={() => handleReservarClick(espacio)}
          >
            Reservar ahora
          </button>
        </div>
    ...
    <div className="flex min-h-screen bg-gray-50 font-sans">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 fixed h-full z-10 hidden md:flex flex-col">
        ...
        <nav className="flex-1 px-4 space-y-2 mt-6">
          {[
            ...
            <button
              key={item.id}
              onClick={() => { setActiveTab(item.id); setSelectedSpace(null); }}
        ...
        <div className="p-4 border-t border-gray-100">
          <button
            onClick={() => { setActiveTab('perfil'); setSelectedSpace(null); }}
            className={`flex items-center gap-3 px-4 py-3 w-full text-left hover:bg-gray-50 rounded-xl transition-all mb-2 ${activeTab === 'perfil' ? 'bg-blue-50 text-[#0099CC]' : 'text-gray-600'}`}
          >
      ...
      <main className="flex-1 md:ml-64 p-8">
        ...
        {selectedSpace ? (
          ...
          /* VISTA 2: DASHBOARD NORMAL */
          <>
            {activeTab === 'inicio' && (
              <div className="max-w-7xl mx-auto animate-fade-in">

                {/* HEADER SIMPLE (Original) */}
                ...
                <div className="bg-white p-2 rounded-2xl shadow-sm border border-gray-100 mb-8">
                  ...
                  <div className="flex gap-2 px-2 pb-2 mt-2 border-t border-gray-100 pt-3 overflow-x-auto">
                    {['Todos', 'Deportivos', 'Sala de Estudio', 'Fab Lab'].map((type) => (
                      <button
                        key={type}
                        onClick={() => setFilterType(type)}
                        className={`px-5 py-2 rounded-full text-sm font-medium transition-all whitespace-nowrap ${filterType === type
